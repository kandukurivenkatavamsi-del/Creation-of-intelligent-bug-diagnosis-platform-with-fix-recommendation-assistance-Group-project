...
export default function SubmitBug() {
  ...
  return (
    <div className="max-w-[1500px]">
      <div className="mb-7 overflow-hidden rounded-3xl border border-violet-400/15 bg-gradient-to-br from-violet-500/[0.08] via-surface to-cyan-400/[0.035] shadow-sm">
        <div className="grid gap-0 lg:grid-cols-[1fr_360px]">
          <div className="p-6 lg:p-7">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <span className="rounded-full border border-violet-400/15 bg-violet-500/[0.08] px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.18em] text-violet-600 dark:text-violet-300">
                Diagnostic Lab
              </span>

              <span className="flex items-center gap-1.5 rounded-full border border-cyan-400/20 bg-cyan-400/[0.08] px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.14em] text-cyan-700 dark:text-cyan-300">
                <span className="h-1.5 w-1.5 rounded-full bg-cyan-500 dark:bg-cyan-400 shadow-[0_0_8px_rgba(34,211,238,0.9)]" />
                5 Agents Ready
              </span>
            </div>
          ...
          <div className="border-t border-slate-200/80 bg-white p-6 lg:border-l lg:border-t-0 dark:border-white/[0.05] dark:bg-white/[0.015]">
            ...
            <div className="mt-3 flex flex-col gap-2">
              {SAMPLE_BUGS.map((bug, i) => {
                const isSelected = form.title === bug.title
                return (
                  <button
                    key={i}
                    type="button"
                    className={`group relative text-left text-xs px-3.5 py-2.5 rounded-xl border transition-all duration-150 font-medium flex items-center justify-between ${
                      isSelected
                        ? 'border-violet-500 border-b-[3px] border-b-violet-700 bg-violet-50 text-violet-900 shadow-[0_6px_16px_rgba(124,92,255,0.25)] -translate-y-0.5 dark:bg-violet-950/50 dark:text-violet-200 dark:border-violet-400'
                        : 'border-slate-200 border-b-[3px] border-b-slate-300 bg-white text-slate-700 shadow-[0_2px_4px_rgba(0,0,0,0.05)] hover:-translate-y-1 hover:border-violet-400 hover:border-b-violet-500 hover:shadow-[0_6px_16px_rgba(124,92,255,0.20)] hover:text-violet-700 hover:bg-violet-50/50 dark:border-white/10 dark:border-b-white/20 dark:bg-white/[0.03] dark:text-slate-300 dark:hover:border-violet-400/60 dark:hover:bg-violet-500/10'
                    } active:translate-y-0.5 active:border-b-[2px] active:shadow-none`}
                    onClick={() => handleSample(bug)}
                    disabled={submitting}
                  >
                    <span className="truncate pr-2 font-medium">
                      {bug.title.length > 38 ? bug.title.slice(0, 38) + '...' : bug.title}
                    </span>
                    <span className={`h-2 w-2 shrink-0 rounded-full transition-all ${
                      isSelected
                        ? 'bg-violet-600 shadow-[0_0_8px_rgba(124,92,255,0.9)]'
                        : 'bg-slate-300 group-hover:bg-violet-400 dark:bg-slate-600'
                    }`} />
                  </button>
                )
              })}
            ...
            <div className="mt-4 flex flex-wrap gap-2.5">
              <button
                type="button"
                className="btn-primary border-b-[3px] border-b-violet-800 shadow-[0_4px_12px_rgba(124,92,255,0.35)] hover:-translate-y-0.5 hover:shadow-[0_6px_18px_rgba(124,92,255,0.45)] active:translate-y-0.5 active:border-b-[2px]"
              ...
              <button
                type="button"
                className="btn-secondary border-b-[3px] border-b-slate-300 shadow-[0_2px_5px_rgba(0,0,0,0.06)] hover:-translate-y-0.5 hover:border-b-slate-400 hover:shadow-[0_4px_10px_rgba(0,0,0,0.1)] active:translate-y-0.5 active:border-b-[2px]"
                onClick={handleClear}
                disabled={submitting}
              >
                Reset
                ...
                {mode === 'code' && (
                  <div className="terminal-panel overflow-hidden">
                    <div className="flex flex-col gap-3 border-b border-white/[0.05] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <select
                        id="language"
                        className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-800 outline-none dark:border-white/[0.06] dark:bg-white/[0.03] dark:text-slate-300"
                        value={language}
                    ...
                    <textarea
                      ...
                      rows={15}
                      spellCheck={false}
                      className="w-full resize-y bg-transparent px-4 py-4 font-mono text-xs leading-6 text-slate-900 dark:text-slate-200 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-600"
...
print(divide(10, 0))`}
                ...
                <div>
                  ...
                  <div className="terminal-panel overflow-hidden">
                    <div className="flex items-center justify-between border-b border-slate-200/80 dark:border-white/[0.05] px-4 py-3">
                      <span className="text-[10px] font-mono uppercase tracking-[0.12em] text-slate-600 dark:text-slate-400 font-semibold">
                        error.log
                      </span>

                      <span className="text-[10px] font-mono text-cyan-600 dark:text-cyan-400/80 font-medium">
                        parser-ready
                      </span>
                    </div>
